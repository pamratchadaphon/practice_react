...
const Ricecrop = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (authResponse.data.status === 'ok') {
          ...
          const farmerResponse = await axios.get('/api/farmer/getFarmers',config)
  ...
  return (
    <UserContext.Provider value={{ fname: farmerbyID[0]?.fname, lname: farmerbyID[0]?.lname }}> 
      <div className='flex'>
        <div className='basis-[16%] h-[100vh]'>
          <Sidebar idFarmer={idAsInt} fname={farmerbyID[0]?.fname} lname={farmerbyID[0]?.lname}/>
        </div>
